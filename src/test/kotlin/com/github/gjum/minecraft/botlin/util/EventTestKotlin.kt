...
class EventTestKotlin {
    ...
    fun eventTest() {
        ...
        // events get handled
        ee.on(AvatarEvents.Spawned::class.java) {
            it.run {
        ...
        ee.on(AvatarEvents.TeleportByServer::class.java) {
            it.run {
        ...
        // events get emitted
        ee.emit(AvatarEvents.TeleportByServer(Vec3d(1.0, 1.0, 1.0), Vec3d.origin, null))
        // events get emitted each time
        ee.emit(AvatarEvents.TeleportByServer(Vec3d(2.0, 2.0, 2.0), Vec3d.origin, null))
